            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/jadwalturne/add`">
                                <Button label="Add New Jadwal Turne" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/jadwalturne'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="id_jadwal" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'jadwalturnelist',
		},
		apiPath: {
			type: String,
			default: 'jadwalturne/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/jadwalturne/view/${data.id_jadwal}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/jadwalturne/edit/${data.id_jadwal}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.id_jadwal) },
			icon: "pi pi-trash"
		}
	]
	}
