            ...
            <section class="page-section mb-3" >
                <div class="container-fluid">
                    <div class="grid justify-content-between align-items-center">
                        ...
                        <div  class="col-fixed " >
                            <router-link :to="`/inventaris/add`">
                                <Button label="Add New Inventari" icon="pi pi-plus" type="button" class="p-button w-full bg-primary "  />
                            </router-link>
                        </div>
                    ...
                    <div  class="col comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/inventaris'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"
                                     :value="records" dataKey="id_inventaris" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        ...
                                        <Column  field="lokasi_barang" header="Lokasi Barang" >
                                            <template #body="{ data, index }">
                                                {{ data.lokasi_barang }}
                                            </template>
                                        </Column>
                                        <Column  field="kode_barang" header="Kode Barang" >
                                            <template #body="{ data, index }">
                                                {{ data.kode_barang }}
                                            </template>
                                        </Column>
                                        <Column  field="kategori_barang" header="Kategori Barang" >
                                            <template #body="{ data, index }">
                                                {{ data.kategori_barang }}
                                            </template>
                                        </Column>
                                        <Column  field="tanggal_memperoleh" header="Tanggal Memperoleh" >
                                            <template #body="{ data, index }">
                                                {{ data.tanggal_memperoleh }}
                                            </template>
                                        </Column>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'inventarislist',
		},
		apiPath: {
			type: String,
			default: 'inventaris/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/inventaris/view/${data.id_inventaris}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/inventaris/edit/${data.id_inventaris}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
